...
class UserEditsOrderTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @order = Order.create(customer: eliza, delivery_time: "2020-08-25 09:00AM", location: "TBD", notes: "Foo")
    visit orders_path
    click_on "August 25, 2020 09:00, Eliza Marcum"
    click_on "Edit Order"
  end
  ...
  test "Order edit happy path" do
    ...
    assert_equal "2020-08-25T09:00:00", field_labeled("Delivery Time").value
    fill_in "Delivery Time", with: "2020-08-25T13:00"
    ...
    assert page.has_css?(".notice", text: "Kate Braddock's order for August 25, 2020 13:00 has been updated.")
    assert_equal current_path, order_path(@order)
    assert page.has_content?("Kate Braddock")
    assert page.has_content?("August 25, 2020")
  ...
  test "Order edit sad path" do
    ...
    # Must fill in date inputs with this date format with Capybara/Racktest.
    # Actual users can use their local date format:
    fill_in "Delivery Time", with: "2020-04-25T14:00"
    fill_in "Location", with: "Neverland"
    fill_in "Notes", with: "Bar"
    click_on "Save Changes"
    assert page.has_css?(".notice", text: "Kate Braddock's order for April 25, 2020 14:00 has been updated.")
    assert_equal current_path, order_path(@order)
    assert page.has_content?("Kate Braddock")
    assert page.has_content?("April 25, 2020")
